...
![mpcplus-ueberzug](img/demo.gif)

`mpcplus-ueberzug` displays mpcplus album art using [ueberzug](https://github.com/seebye/ueberzug). It works on `alacritty`, `st`, `urxvt`,  `kitty`, `xterm` and `lxterm`. Unlike existing scripts, it dynamically sizes and positions the cover art such that the window can be any size, even resized. It has many settings to customize the position of the album art to suit your mpcplus setup.

**mpcplus-ueberzug is currently gradually being improved, expect more features in each version but also potential changes in how settings work. I will update this message when this script is feature-complete.**

## Install

Follow the instructions on the [Setup](https://github.com/tam-carre/ncmpcpp-ueberzug/wiki/Setup) wiki page.
...
#### Require manually setting character size in `mpcplus_cover_art.sh`:
* `cool-retro-term` 
* `sakura` (resizing can break geometry)
* `gnome-terminal`, `terminator`, `xfce4-terminal`  (Album art displays on the last opened terminal irrespective of which is the mpcplus window, resizing can break geometry)
...
* Change cleanup mechanism to allow for several mpcplus-ueberzug instances
